...
```cpp
// 重点看 init() - 创建socket和绑定地址
// 和 receiveLoop() - 接收线程（阻塞接收）
```
...
| Socket API | ⭐⭐ 中等 | 了解socket基础，重点关注recvfrom/sendto |
| 回调机制 | ⭐⭐ 中等 | 理解数据流向 |
| 位操作 | ⭐⭐ 中等 | 用计算器验证 |
...
2. **Socket编程**：了解基础socket API（socket, bind, sendto, recvfrom）
3. **C++11**：了解 std::thread, lambda
