...
使用原生socket + 独立线程实现，不依赖Boost.Asio。

```cpp
bool init(local_ip, local_port, remote_ip, remote_port);  // 初始化socket
bool send(data, len);                                      // 发送数据
void setCallback(callback);                               // 设置接收回调
void startReceive();                                      // 启动接收线程
void close();                                             // 关闭连接
```

**实现方式**：
- 创建UDP socket，绑定本地地址
- 在独立线程中循环调用 `recvfrom()` 阻塞接收
- 收到数据后通过回调函数传递给上层
- 发送使用 `sendto()` 直接发送
